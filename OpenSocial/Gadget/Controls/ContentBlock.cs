...
//		private Dictionary<string, OsTemplate> _templateHash = new Dictionary<string, OsTemplate>(15);
		...
		#region Template management support

		/// <summary>
		/// Add a generic, unnamed template
		/// </summary>
		/// <returns></returns>
		public OsTemplate AddTemplate()
		{
			return AddTemplate(null);
		}

		///// <summary>
		///// Generate an anonymous template name to use as a key in the template hash.
		///// </summary>
		///// <returns></returns>
		//string GenerateTemplateName()
		//{
		//    int val = _templateHash.Count;
		//    string templateName = OsTemplate.ANON_NAMEPART + val.ToString().PadLeft(3, '0');
		//    if (_templateHash.ContainsKey(templateName))
		//    {
		//        if (_templateHash.Count > 999)
		//        {
		//            throw new Exception("Anonymous templates are not allowed when there are 1000 or more templates");
		//        }
		//        do
		//        {

		//            templateName = OsTemplate.ANON_NAMEPART + (++val).ToString().PadLeft(3, '0');
		//        } while (_templateHash.ContainsKey(templateName));
		//    }
		//    return templateName;
		//}

		/// <summary>
		/// Creates an OsTemplate scoped for the current ContentBlock
		/// </summary>
		/// <returns></returns>
		public BaseContainerControl CreateLocalTemplate()
		{
			OsTemplate template = new OsTemplate();
			template.MyRootMaster = this.MyRootMaster;
			return template;
		}
		/// <summary>
		/// Creates an OsTemplate scoped for the current ContentBlock
		/// </summary>
		/// <param name="tag">Custom tag definition.  Null if this is not a tag template</param>
		/// <returns></returns>
		public BaseContainerControl CreateLocalTemplate(string tag)
		{
			if (string.IsNullOrEmpty(tag))
			{
				return CreateLocalTemplate();
			}
			CustomTagTemplate template = new OsTagTemplate(tag);
			template.MyRootMaster = this.MyRootMaster;
			return template;
		}
		/// <summary>
		/// Creates a DataScript scoped for the current ContentBlock
		/// </summary>
		/// <returns></returns>
		public DataScript CreateLocalDataScript()
		{
			DataScript script = new DataScript();
			script.MyRootMaster = this.MyRootMaster;
			return script;
		}
		...
		/// <param name="offset"></param>
		/// <returns></returns>
		public DataScript CreateLocalDataScript(string markup, OffsetItem offset)
		{
			DataScript script = new DataScript(markup, offset, MyRootMaster);
			return script;
		}

		/// <summary>
		/// Adds a new DataScript to the internal DataScripts list
		/// </summary>
		/// <remarks>Questionable if this should be left or deprecated in favor of AddControl</remarks>
		/// <param name="dataScript"></param>
		/// <returns></returns>
		public DataScript AddDataScript(DataScript dataScript)
		{
			if (dataScript != null)
			{
				if (dataScript.ViewMask == 0)
				{
					dataScript.ViewMask = this.ViewMask;
				}
				DataScripts.Add(dataScript);
				if (!dataScript.IsParsed)
				{
					dataScript.Parse();
				}
			}
		...
		/// <summary>
		/// Adds a new, empty OsTemplate to the internal Templates hash.
		/// </summary>
		/// <param name="name"></param>
		/// <returns></returns>
		[Obsolete("We're not using template names")]
		public OsTemplate AddTemplate(string name)
		{
			/*
			...
			OsTemplate t = new OsTemplate(name);
			return AddTemplate(name, t);
		}

		/// <summary>
		/// Adds a template to the TemplateHash
		/// </summary>
		/// <param name="name"></param>
		/// <param name="template"></param>
		/// <returns></returns>
		public OsTemplate AddTemplate(string name, OsTemplate template)
		{
			if (template == null)
			{
				throw new ArgumentNullException("Must specify template");
			}
			/*
			if (String.IsNullOrEmpty(name))
			{
		...
		///// <summary>
		///// Verifies that the name is unique in the template hash.
		///// Throws an exception if the name is already in use.
		///// </summary>
		///// <param name="name"></param>
		//private void AssertUniqueTemplateName(string name)
		//{
		//    if (_templateHash.ContainsKey(name))
		//    {
		//        throw new Exception("Template key already in use: " + name);
		//    }
		//}
